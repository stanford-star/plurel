...
def test_database_adds_primary_keys_and_relbench_metadata(schema):
    ...
    db = database(schema, frames, times={"orders": "when"})
    assert set(db.table_dict) == set(ROWS)
    customers, orders = db.table_dict["customers"], db.table_dict["orders"]
    assert (
        customers.pkey_col == "id"
        and customers.time_col is None
        and customers.fkey_col_to_pkey_table == {}
    )
    assert orders.time_col == "when" and orders.fkey_col_to_pkey_table == {
        "customer_id": "customers"
    }
    np.testing.assert_array_equal(customers.df["id"], np.arange(ROWS["customers"]))
    linked = orders.df["customer_id"].dropna().astype(int)
    assert linked.isin(customers.df["id"]).all() and orders.df["customer_id"].isna().any()
    assert (
        db.min_timestamp == orders.df["when"].min() and db.max_timestamp == orders.df["when"].max()
    )
    pd.testing.assert_frame_equal(orders.df.drop(columns="id"), frames["orders"])
    for bad in (
        {"times": {"orders": "amount"}},
        {"times": {"shops": "when"}},
        {"pkey": "amount"},
...
def test_write_and_read_round_trip(schema, tmp_path):
    db = database(schema, schema.sample(ROWS, seed=0), times={"orders": "when"})
    ...
    assert (path / "manifest.yaml").exists() and {p.name for p in (path / "db").iterdir()} == {
        "customers.parquet",
        "orders.parquet",
    }
    loaded = read(path)
    for name, table in db.table_dict.items():
        ...
        assert (again.pkey_col, again.time_col, again.fkey_col_to_pkey_table) == (
            table.pkey_col,
            table.time_col,
    ...
    assert (
        loaded.table_dict["orders"].df["customer_id"].isna().sum()
        == db.table_dict["orders"].df["customer_id"].isna().sum()
    )
